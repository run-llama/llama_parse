...
def parse(file: str, **kwargs: Dict[str, Any]) -> None:
    """
    Parse the given file and output the result to STDOUT
    ...
    click.echo(
        json.dumps(parser.get_json_result(file))
        if kwargs["result_type"] == ResultType.JSON
        else parser.load_data(file)[0].text
    )
